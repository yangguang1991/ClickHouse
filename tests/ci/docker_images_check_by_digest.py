...
def main():
    ...
    for image in images_build_list:
        if image.repo not in missing_images:
            continue
        logging.info("Start building image: {}" % image)

        image_versions = (
            [image_digests[image.repo]]
            if not args.suffix
            else [f"{image_digests[image.repo]}-{args.suffix}"]
        )
        ...
        res = process_single_image(
            ...
            additional_cache,
            args.push,
            from_tag=image_digests[image.parent] if image.parent else None,
        )
        test_results += res
    ...
    description = format_description(
        f"Images build done [ok: {ok_cnt}, failed: {fail_cnt}]"
    )
